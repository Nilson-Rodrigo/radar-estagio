...
const Filters: React.FC<FiltersProps> = ({ filters, opcoes, onAlterarFiltro, onLimparFiltros }) => {
  const baseSelect =
    'px-4 py-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-sm text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-200';
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <button
        ...
        className="px-4 py-2 rounded-xl text-sm font-semibold text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-all duration-200"
      >
        Limpar Filtros
      </button>
    </div>
